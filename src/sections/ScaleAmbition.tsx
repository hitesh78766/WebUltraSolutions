      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 w-full relative z-10">
        ...
        <div className="ambition-header text-center max-w-3xl mx-auto mb-14 space-y-4">
          <h2 className="text-3xl md:text-4xl lg:text-[42px] font-bold text-[#111827] tracking-tight leading-[1.1]">
            Scale With the <span className="text-[#F97316]">Power of Your Ambition</span>
          </h2>
          <div className="w-12 h-1 bg-[#F97316] mx-auto rounded-full mt-2" />
          <p className="text-slate-500 text-[13px] md:text-[14px] font-medium leading-relaxed max-w-2xl mx-auto pt-2">
          ...
          {tiers.map((tier, i) => (
            <div key={i} className={`ambition-card bg-white border ${tier.color} rounded-md p-5 lg:p-6 flex flex-col h-full group hover:shadow-2xl transition-all duration-500 relative`}>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className={`w-9 h-9 rounded-md ${tier.popular ? 'bg-[#F97316] text-white' : 'bg-slate-900 text-white'} flex items-center justify-center shadow-lg`}>
                  <tier.icon size={18} />
                </div>
                {tier.popular && (
              ...
              <h3 className="text-lg font-bold text-[#111827] mb-4 tracking-tight">{tier.title}</h3>

              <div className="space-y-3 flex-grow mb-6">
                <p className="text-[#F97316] text-[11px] font-medium italic leading-snug tracking-tight">
                  {tier.tagline}
                </p>
                ...
                <ul className="space-y-2 pt-2">
                  {tier.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-2.5 group/item">
                      <div className="mt-0.5 w-4.5 h-4.5 rounded-full bg-[#F97316]/10 flex items-center justify-center shrink-0">
                        <Check size={10} className="text-[#F97316]" strokeWidth={3} />
                      </div>
                      <span className="text-[10.5px] font-bold text-slate-700 tracking-tight leading-normal group-hover:text-slate-900 transition-colors pt-0.5">{feature}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              {/* Action Button */}
              <button className={`w-full py-3.5 rounded-md flex items-center justify-center gap-2 font-bold text-[9px] uppercase tracking-[0.2em] transition-all active:scale-95 group/btn ${tier.popular ? 'bg-[#F97316] text-white hover:bg-[#EA580C] shadow-orange-500/20 shadow-xl' : 'bg-[#F97316] text-white hover:bg-[#EA580C]'}`}>
