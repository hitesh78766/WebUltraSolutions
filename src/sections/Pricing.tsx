      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 w-full">
        ...
        <div className="text-center space-y-4 mb-20">
          <div className="reveal-up inline-flex items-center gap-3 px-4 py-1.5 bg-indigo-500/10 rounded-full border border-indigo-500/5">
            ...
            <span className="text-[11px] font-bold text-indigo-700 uppercase tracking-[0.25em]">Investment</span>
          </div>
          <h2 className="reveal-up text-2xl md:text-3xl lg:text-5xl font-display font-bold text-slate-900 leading-tight">
            ...
            >
              {p.popular && (
                <div className="absolute -top-5 left-1/2 -translate-x-1/2 px-6 py-2 bg-indigo-600 rounded-full shadow-lg">
                  <span className="text-[10px] font-bold text-white uppercase tracking-widest">Most Popular</span>
                </div>
              )}
              ...
              <div className="space-y-6 flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-slate-300 uppercase tracking-widest">#{i+1}</span>
                </div>

                <div>
                  <h3 className="text-2xl font-display font-bold text-slate-900">{p.name}</h3>
                  <p className="text-slate-400 text-xs font-medium mt-1 leading-relaxed">{p.description}</p>
                </div>

                <div className="flex items-baseline gap-1">
                   <span className="text-sm font-bold text-slate-400">$</span>
                   <span className="text-5xl font-bold text-slate-900 leading-none">{p.price}</span>
              ...
              <div className="pt-10">
                <button className={`w-full py-4 rounded-xl font-bold text-sm uppercase tracking-widest transition-all ${p.popular ? 'bg-slate-900 text-white shadow-xl hover:bg-slate-800' : 'bg-slate-50 text-slate-900 hover:bg-slate-100 border border-slate-100'}`}>
                  Get Started
                </button>
              </div>
            </div>
